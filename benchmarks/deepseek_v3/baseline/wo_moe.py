...
from benchmarks.deepseek_v3.baseline.util import (GB, config, get_moe_weights,
                                                  hf_config,
                                                  load_fused_moe_weights,
                                                  offloading, quant_config)
from wde.tasks.decode_only.modelzoo.deepseek_v2 import DeepseekV2RemoteMoE


def init():
    weights = get_moe_weights(wo_moe=True)

    prefix = "model.layers.3.mlp."

    with torch.device(config.device):
        model = DeepseekV2RemoteMoE(config=hf_config,
                                    quant_config=quant_config,
                                    prefix=prefix)

        load_fused_moe_weights(model, weights, prefix=prefix)
